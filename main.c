...
void myInit()
{
	...
	texture = SOIL_load_OGL_texture
    	...
    	if( 0 == texture)
		{
			printf( "SOIL player image loading error: '%s'\n", SOIL_last_result() );
		}
    ...
    if( 0 == end)
	{
		printf( "SOIL end image loading error: '%s'\n", SOIL_last_result() );
	}
	glBindTexture(GL_TEXTURE_2D, end);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    ...
    if( 0 == back)
	{
		printf( "SOIL back image loading error: '%s'\n", SOIL_last_result() );
	}
    ...
    if( 0 == start)
	{
		printf( "SOIL start image loading error: '%s'\n", SOIL_last_result() );
	}
	glBindTexture(GL_TEXTURE_2D, start);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
